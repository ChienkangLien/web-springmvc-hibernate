...
function loadNewRoomOrder() {
  ...
  fetch("/web-springmvc-hibernate/admin/room/RoomOrderController/userSearch")
    .then((resp) => {
      if (resp.status === 204) {
...
$(document).on("click", "button.editBtn", function () {
  ...
  fetch(
    `/web-springmvc-hibernate/admin/room/RoomOrderController/orderIdSearch?orderId=${orderId}`
  )
    .then((resp) => {
      if (resp.status === 204) {
        console.log("resp.status===" + resp.status);
    ...
    .then((body) => {
      try {
        if (body != null) {
          ...
          let html = `<div class="mb-3">
            ...
            <input type="date" data-date='${
              body.orderStartDate
            }' class="form-control order_start_date" name="orderStartDate" id="orderStartDate" value="${
            body.orderStartDate
        ...
        <div class="mb-3">
            ...
            <input type="date" data-date='${
              body.orderEndDate
            }' class="form-control order_end_date" name="orderEndDate" id="orderEndDate" value="${
            body.orderEndDate
          }"${body.orderStatus == "完成單" ? "readonly" : ""}>
        </div>
...
$(document).on("click", ".deleteBtn", function () {
  ...
  if (confirm("確認刪除?")) {
    ...
    fetch("/web-springmvc-hibernate/admin/room/RoomOrderController", {
      method: "DELETE",
      headers: {
...
$(document).on("change", ".order_end_date, .order_start_date", function () {
  ...
  if (startDate != "" && endDate != "") {
    fetch(
      `/web-springmvc-hibernate/admin/room/RoomController/check?startDate=${startDate}&endDate=${endDate}&roomId=${roomId}&orderId=${orderId}`
    )
      .then((resp) => {
        if (resp.status === 204) {
          console.log("resp.status===" + resp.status);
...
$(document).on("click", "button.confirmEdit", function () {
  ...
  if (ava && !hasError) {
    fetch(
      "/web-springmvc-hibernate/admin/room/RoomOrderController?action=edit",
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json;charset=UTF-8",
