...
function loadHistoryRoomOrder() {
  ...
  fetch(
    "/web-springmvc-hibernate/admin/room/RoomOrderController?status=maintain&limit=0"
  )
    .then((resp) => {
      if (resp.status === 204) {
    ...
    .then((body) => {
      ...
      <td>${body[i].administratorVO.adminName}</td>
...
$(document).ready(function () {
  ...
  //載入房型名稱到標籤
  fetch("/web-springmvc-hibernate/admin/room/RoomTypeController/getAll")
    .then((resp) => {
      if (resp.status === 204) {
        console.log("resp.status===" + resp.status);
      } else {
    ...
    .then((body) => {
      ...
      fetch(`RoomController/search?roomTypeId=${trId}`)
        .then((resp) => {
          if (resp.status === 204) {
            console.log("resp.status===" + resp.status);
          } else {
...
$(document).on(
  ...
  function () {
    ...
    if (startDate != "" && endDate != "" && typeId != "") {
      ...
      fetch(
        `/web-springmvc-hibernate/admin/room/RoomController/available?startDate=${startDate}&endDate=${endDate}&typeId=${typeId}`
      )
        .then((resp) => {
          if (resp.status === 204) {
            console.log("resp.status===" + resp.status);
...
$(document).on("click", "button#selectBtn", function () {
  ...
  fetch(
    `/web-springmvc-hibernate/admin/room/RoomOrderController/calendar?roomId=${roomId}`
  )
    .then((resp) => {
      if (resp.status === 204) {
        console.log("resp.status===" + resp.status);
      ...
      try {
        ...
        if (body.length != null) {
          for (let i = 0; i < body.length; i++) {
            const newObject = {
              title: `${
                body[i].memberVO.userName == null
                  ? ""
                  : "會員:" + body[i].memberVO.userName + " "
              }${
                body[i].orderResident == null
                  ? ""
                  : "入住人:" + body[i].orderResident + " "
              }${
                body[i].administratorVO.adminName == null
                  ? ""
                  : "關房員工:" + body[i].administratorVO.adminName + " "
              }${
                body[i].orderRemark == null ? "" : "備註:" + body[i].orderRemark
              }`,

              start: body[i].orderStartDate,
...
function formatEndDate(dateStr) {
  ...
  return date.toISOString().slice(0, 10); // 格式化日期字串;
}
